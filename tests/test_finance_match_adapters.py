...
from app.finance_match_adapters import invoice_candidates, obligation_candidates, proposals_for_transaction, rental_candidates
...
class FinanceMatchAdapterTests(unittest.TestCase):
    ...
    @patch("app.finance_match_adapters.RentalBillingStore")
    @patch("app.finance_match_adapters.ContactStore")
    def test_rental_candidates_respect_contact_acl(self, contact_store, rental_store):
        rental_store.return_value.ledger.return_value = [
            {"ledger_id": "rent-1", "contact_id": "c-visible", "booked_on": "2026-09-01", "kind": "payment", "amount": "-500.00", "note": "Miete"},
            {"ledger_id": "rent-2", "contact_id": "c-hidden", "booked_on": "2026-09-01", "kind": "payment", "amount": "-500.00", "note": "Miete"},
        ]
        def get_contact(contact_id, actor=""):
            if contact_id == "c-hidden":
                raise ValueError("contact is not shared with this user")
            return {"fields": {"display_name": "Sichtbarer Mieter"}}
        contact_store.return_value.get.side_effect = get_contact
        candidates = rental_candidates(self.root, "jens")
        self.assertEqual(["rent-1"], [item.source_id for item in candidates])
        self.assertEqual("Sichtbarer Mieter", candidates[0].counterparty_name)
