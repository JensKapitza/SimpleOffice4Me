#!/usr/bin/env python3
"""Build, clone and apply SimpleOffice4Me releases without Git on the target."""
...
def _offline_restart(root: Path) -> None:
    """Restart using the already prepared venv; never invoke pip or start scripts."""
...
def main() -> int:
    ...
    build = commands.add_parser("build", help="Create a Git-independent self-deploy release archive")
    ...
    clone = commands.add_parser("clone", help="Install SimpleOffice on another PC from an archive")
    clone.add_argument("archive")
    clone.add_argument("target")
    clone.add_argument("--offline-install", action="store_true")

    update = commands.add_parser("update", help="Apply an archive to an existing installation")
