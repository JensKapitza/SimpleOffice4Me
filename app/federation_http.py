...
def authenticate():
    if not _authorized():
        return Response("federation authentication required\n", 401, {
            "WWW-Authenticate": 'Bearer realm="SimpleOffice4Me Federation"',
...
def _send(path: Path, digest: str) -> Response:
    ...
    if request.headers.get("If-None-Match", "").strip() == etag and not request.headers.get("Range"):
...
def capabilities():
    return jsonify({
        "protocol": "sofp", "versions": [1], "transport": "http",
        "hashes": ["sha256"], "range": True, "curl_resume": True,
...
def blob(digest: str):
    digest = digest.removeprefix("sha256:").casefold()
    if not re.fullmatch(r"[0-9a-f]{64}", digest):
        return jsonify({"error": "invalid_hash"}), 400
    store = _store()
    store.initialize()
    with store._db() as db:
        row = db.execute("SELECT relative_path FROM scan_file WHERE sha256=? ORDER BY relative_path LIMIT 1", (digest,)).fetchone()
    if row is None:
        return jsonify({"error": "not_found"}), 404
    path = (store.root / str(row["relative_path"])).resolve()
    if store.root not in (path, *path.parents) or not path.is_file() or path.is_symlink():
        return jsonify({"error": "not_found"}), 404
    return _send(path, digest)
