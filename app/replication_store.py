...
class ReplicationStore:
    ...
    def add_target(self, values: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        if not label or not path.is_dir():
            raise ValueError("Zielname und vorhandener Zielordner sind erforderlich")
        path = path.resolve()
        if path == self.root or self.root in path.parents or path in self.root.parents:
            raise ValueError("Hauptdatenverzeichnis und Unterordner dürfen kein Spiegelungsziel sein")
        ...
        data = self.status()
        target = {
            "target_id": str(uuid.uuid4()), "label": label, "path": str(path), "created_at": utc_now(),
            "created_by": actor, "enabled": True, "last_run": None, "last_error": "",
            "initial_import": initial_import, "last_import": initial_import,
        }
        data["targets"].append(target)
        self._save(data, actor, "replication_target_created", target["target_id"])
        return target

    def set_target_enabled(self, target_id: str, enabled: bool, actor: str) -> dict[str, Any]:
        data = self.status()
        target = next((item for item in data["targets"] if item["target_id"] == target_id), None)
        if target is None:
            raise ValueError("Unbekanntes Spiegelungsziel")
    ...
    def import_target(self, target_id: str, actor: str) -> dict[str, Any]:
        ...
        if target.get("enabled", True) is False:
            raise ValueError("Spiegelungsziel ist pausiert")
    ...
    def add_rule(self, values: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        if not any(item["target_id"] == target_id for item in data["targets"]):
            raise ValueError("Unbekanntes Spiegelungsziel")
        categories = self._categories(values.get("categories", []))
        label = str(values.get("label", "")).strip()
        if not label or not categories:
            raise ValueError("Name und mindestens eine Datenklasse sind erforderlich")
        rule = {
            "rule_id": str(uuid.uuid4()), "label": label, "target_id": target_id,
            "categories": categories, "tags": self._tags(values.get("tags", "")),
            "enabled": True, "last_run": None, "last_result": {},
        }
        data["rules"].append(rule)
        self._save(data, actor, "replication_rule_created", rule["rule_id"])
        return rule

    def run_rule(self, rule_id: str, actor: str) -> dict[str, Any]:
        data = self.status()
        rule = next((item for item in data["rules"] if item["rule_id"] == rule_id), None)
        if not rule:
            raise ValueError("Unbekannte Spiegelungsregel")
        target = next(item for item in data["targets"] if item["target_id"] == rule["target_id"])
        ...
        rule["last_run"] = result["at"]
        rule["last_result"] = result
        target["last_run"] = result["at"]
        target["last_error"] = ""
        self._save(data, actor, "replication_completed", rule_id)
        return result
    ...
    def run_all(self, actor: str) -> dict[str, Any]:
        ...
        for rule in self.status()["rules"]:
            if not rule.get("enabled", True):
                continue
            try:
                results.append({"rule_id": rule["rule_id"], **self.run_rule(rule["rule_id"], actor)})
    ...
    def add_restic_repository(self, values: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        if not label or not repository:
            raise ValueError("Name und restic-Repository sind erforderlich")
        data = self.status()
        item = {
            "repository_id": str(uuid.uuid4()), "label": label, "repository": repository,
            "categories": self._categories(values.get("categories", [])) or list(CATEGORIES),
            "tags": self._tags(values.get("tags", "")), "last_run": None, "last_error": "",
        }
        data["restic"].append(item)
    ...
    def run_restic(self, repository_id: str, action: str, password: str, actor: str, restore_path: str = "") -> dict[str, Any]:
        ...
        if not item:
            raise ValueError("Unbekanntes restic-Repository")
        executable = shutil.which("restic")
        if not executable:
            raise ValueError("restic ist nicht installiert")
        if not password:
            raise ValueError("Restic-Passwort wird für diesen Lauf benötigt und nicht gespeichert")
        env = {"PATH": os.environ.get("PATH", ""), "RESTIC_PASSWORD": password}
        command = [executable, "-r", item["repository"]]
        if action == "init":
            command += ["init"]
        elif action == "check":
            command += ["check"]
        ...
        self._save(data, actor, "restic_" + action, repository_id)
    ...
    def _tags(value: Any) -> list[str]:
        return [item.strip() for item in (value.split(",") if isinstance(value, str) else value or []) if item.strip()]

    @staticmethod
    def _categories(value: Any) -> list[str]:
        return [item for item in value if item in CATEGORIES]

    def _initialize(self) -> None:
        self.control.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            atomic_json_write(self.path, {"targets": [], "rules": [], "restic": []})

    def _read(self) -> dict[str, Any]:
        try:
            data = json.loads(self.path.read_text(encoding="utf-8"))
            return data if isinstance(data, dict) else {"targets": [], "rules": [], "restic": []}
        except (OSError, json.JSONDecodeError):
            return {"targets": [], "rules": [], "restic": []}
    ...
    def _save(self, data: dict[str, Any], actor: str, action: str, object_id: str) -> None:
        with exclusive_file_lock(self.control / ".replication-write.lock"):
            ...
            self.history.record(action, actor, "replication", object_id, data)
...
def run_replications_command(actor: str) -> None:
    ...
    if result["errors"]:
        raise click.ClickException("one or more replications failed")
...
def init_app(app) -> None:
    ...
    printershare_quickwins.init_app(app)
