...
class ContactManagement:
    ...
    def duplicate_candidates(self, actor: str, minimum_score: int = 70) -> list[DuplicateCandidate]:
        contacts = self.store.contacts(actor)
        candidates: list[DuplicateCandidate] = []
        for index, left in enumerate(contacts):
            for right in contacts[index + 1:]:
                score, reasons = self._duplicate_score(left, right)
                if score >= minimum_score:
                    candidates.append(DuplicateCandidate(left, right, score, tuple(reasons)))
        candidates.sort(key=lambda item: (-item.score, self._norm_text(item.left.get("fields", {}).get("display_name"))))
        return candidates

    def _duplicate_score(self, left: dict[str, Any], right: dict[str, Any]) -> tuple[int, list[str]]:
        ...
        if email_l and email_l == email_r:
            score += 70; reasons.append("gleiche E-Mail")
        if phone_l and len(phone_l) >= 6 and phone_l == phone_r:
            score += 60; reasons.append("gleiche Telefonnummer")
        if name_l and name_l == name_r:
            score += 35; reasons.append("gleicher Anzeigename")
        if company_l and company_l == company_r:
            score += 10; reasons.append("gleiche Firma")
        ...
        if left_name_parts and left_name_parts == right_name_parts and name_l != name_r:
            score += 25; reasons.append("gleicher Vor-/Nachname")
    ...
    def merge(self, target_id: str, source_id: str, actor: str) -> dict[str, Any]:
        ...
        with exclusive_file_lock(self.store.control / ".contacts-write.lock"):
            ...
            merged_fields = dict(source.get("fields", {}))
            merged_fields.update({key: value for key, value in target.get("fields", {}).items() if str(value).strip()})
            merged["fields"] = merged_fields
            address_keys: set[tuple[str, str]] = set()
            addresses: list[dict[str, Any]] = []
            for address in [*target.get("addresses", []), *source.get("addresses", [])]:
                key = (self._norm_text(address.get("label")), self._norm_text(address.get("value")))
                if key in address_keys:
                    continue
                address_keys.add(key); addresses.append(copy.deepcopy(address))
            merged["addresses"] = addresses
            merged["tags"] = self._clean_labels([*target.get("tags", []), *source.get("tags", [])])
            merged["groups"] = self._clean_labels([*target.get("groups", []), *source.get("groups", [])])
            merged["managers"] = sorted(set(target.get("managers", [])) | set(source.get("managers", [])), key=str.casefold)
            merged["changes"] = [*source.get("changes", []), *target.get("changes", [])][-200:]
    ...
    def import_preview(self, text: str, actor: str) -> dict[str, Any]:
        """Parse a small CSV-like contact import without writing any data.

        The preview accepts semicolon or comma separated UTF-8 data with a
        header row. It intentionally performs no automatic merge.
        """
