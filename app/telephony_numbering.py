...
def clean_number(value: Any, label: str = "Nummer") -> str:
    number = "".join(ch for ch in str(value or "").strip() if ch.isdigit())
...
def private_dial_number(master_prefix: str, base_number: str, extension: str) -> str:
    """Human dial string only; it is deliberately not claimed to be E.164/PSTN."""
...
class MasterNumberRegistry:
    ...
    def resolve_master_prefix(self, dialed: str) -> dict[str, Any]:
        digits = clean_number(str(dialed).lstrip("+"), "Federation-Ziel")
        with self._db() as db:
            rows = db.execute(
