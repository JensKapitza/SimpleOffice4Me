"""Receive signed stage-1 chat messages and attachment copies from federation peers."""
...
def receive_event():
    try:
        ...
        chat=_chat(); chat.upsert_remote_room(room_id,str(room_data.get("title") or "Chat"),peer["peer_id"],targets,sources); chat.upsert_remote_message(room_id,peer["peer_id"],sender,message_id,str(msg.get("body") or ""),message_type=str(msg.get("message_type") or "text"),payload=msg.get("payload") if isinstance(msg.get("payload"),dict) else {},created_at=int(msg.get("created_at") or 0) or None)
