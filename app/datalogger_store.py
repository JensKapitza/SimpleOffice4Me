...
class DataLoggerStore:
    ...
    @staticmethod
    def _sample_range_clause(start, end) -> tuple[str, tuple]:
        if start and end:
            return "channel_id=? AND measured_at>=? AND measured_at<=?", (start, end)
        if start:
            return "channel_id=? AND measured_at>=?", (start,)
        if end:
            return "channel_id=? AND measured_at<=?", (end,)
        return "channel_id=?", ()

    def sample_count(self, channel_id, start=None, end=None):
        if start and end:
            query = "SELECT COUNT(*) FROM metric_sample WHERE channel_id=? AND measured_at>=? AND measured_at<=?"
            args = (channel_id, start, end)
        elif start:
            query = "SELECT COUNT(*) FROM metric_sample WHERE channel_id=? AND measured_at>=?"
            args = (channel_id, start)
        elif end:
            query = "SELECT COUNT(*) FROM metric_sample WHERE channel_id=? AND measured_at<=?"
            args = (channel_id, end)
        else:
            query = "SELECT COUNT(*) FROM metric_sample WHERE channel_id=?"
            args = (channel_id,)
        with self.connect() as db:
    ...
    def samples(self, channel_id, limit=1000, start=None, end=None, offset=0):
        ...
        base = """SELECT sample.measured_at,sample.value,sample.quality,sample.source_id,
        ...
        """
        if start and end:
            query = base + " WHERE sample.channel_id=? AND sample.measured_at>=? AND sample.measured_at<=? ORDER BY sample.measured_at DESC LIMIT ? OFFSET ?"
            args = (channel_id, start, end, limit, offset)
        elif start:
            query = base + " WHERE sample.channel_id=? AND sample.measured_at>=? ORDER BY sample.measured_at DESC LIMIT ? OFFSET ?"
            args = (channel_id, start, limit, offset)
        elif end:
            query = base + " WHERE sample.channel_id=? AND sample.measured_at<=? ORDER BY sample.measured_at DESC LIMIT ? OFFSET ?"
            args = (channel_id, end, limit, offset)
        else:
            query = base + " WHERE sample.channel_id=? ORDER BY sample.measured_at DESC LIMIT ? OFFSET ?"
            args = (channel_id, limit, offset)
        with self.connect() as db:
