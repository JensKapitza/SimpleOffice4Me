...
def content_defined_ranges(path: Path, *, min_size: int = DEFAULT_MIN_BLOCK, avg_size: int = DEFAULT_AVG_BLOCK, max_size: int = DEFAULT_MAX_BLOCK) -> Iterator[tuple[int, int]]:
    ...
    with Path(path).open("rb") as source:
        while True:
            ...
            for byte in data:
                ...
                if length >= minimum and ((len(window) == ROLLING_WINDOW and (fingerprint & mask) == 0) or length >= maximum):
...
def build_content_manifest(path: Path, *, min_size: int = DEFAULT_MIN_BLOCK, avg_size: int = DEFAULT_AVG_BLOCK, max_size: int = DEFAULT_MAX_BLOCK) -> dict[str, Any]:
    path = Path(path)
    minimum, average, maximum = _sizes(min_size, avg_size, max_size)
    blocks: list[dict[str, Any]] = []
    whole = hashlib.sha512()
    with path.open("rb") as source:
        for index, (offset, length) in enumerate(content_defined_ranges(path, min_size=minimum, avg_size=average, max_size=maximum)):
            source.seek(offset)
    ...
    return {"schema": SCHEMA, "hash_algorithm": HASH_ALGORITHM, "file_sha512": whole.hexdigest(), "size": path.stat().st_size, "min_block_size": minimum, "avg_block_size": average, "max_block_size": maximum, "rolling_window": ROLLING_WINDOW, "block_count": len(blocks), "blocks": blocks}
...
def missing_block_hashes(manifest: dict[str, Any], available: set[str]) -> list[str]:
    ...
    for block in manifest.get("blocks", []):
        ...
        if digest not in normalized and digest not in seen:
            result.append(digest); seen.add(digest)
    return result


def reusable_bytes(manifest: dict[str, Any], available: set[str]) -> int:
    normalized = {normalize_sha512(value) for value in available}
    return sum(int(block["length"]) for block in manifest.get("blocks", []) if normalize_sha512(block["sha512"]) in normalized)
...
class FederationBlockStore:
    ...
    def initialize(self) -> None:
        self.control.mkdir(parents=True, exist_ok=True); self.cache.mkdir(parents=True, exist_ok=True)
        with self._db() as db:
            db.executescript("""
    ...
    def _relative(self, path: Path) -> str:
        ...
        try:
            ...
            raise ValueError("block source outside document root") from exc

    def needs_index(self, path: Path) -> bool:
        path = Path(path).resolve(); relative = self._relative(path); stat = path.stat()
        with self._db() as db:
    ...
    def manifest_for_file(self, path: Path, *, force: bool = False) -> dict[str, Any]:
        path = Path(path).resolve(); relative = self._relative(path); stat = path.stat()
        if not force:
            with self._db() as db: row = db.execute("SELECT * FROM file_manifest WHERE relative_path=?", (relative,)).fetchone()
            if row and int(row["file_size"]) == stat.st_size and int(row["file_mtime_ns"]) == stat.st_mtime_ns:
                try:
                    manifest = json.loads(row["manifest_json"])
                    if content_manifest_valid(manifest): return manifest
                except json.JSONDecodeError: pass
        manifest = build_content_manifest(path); self.register_manifest(path, manifest); return manifest

    def register_manifest(self, path: Path, manifest: dict[str, Any]) -> None:
        if not content_manifest_valid(manifest): raise ValueError("invalid content block manifest")
        path = Path(path).resolve(); relative = self._relative(path); stat = path.stat(); indexed_at = int(time.time())
        with self._db() as db:
            db.execute("DELETE FROM block_source WHERE relative_path=?", (relative,))
            db.executemany("INSERT OR REPLACE INTO block_source(sha512,relative_path,offset,length,file_size,file_mtime_ns,indexed_at) VALUES(?,?,?,?,?,?,?)", [(normalize_sha512(block["sha512"]), relative, int(block["offset"]), int(block["length"]), stat.st_size, stat.st_mtime_ns, indexed_at) for block in manifest["blocks"]])
    ...
    def available(self, hashes) -> set[str]:
        requested=[]
        for value in hashes:
            try: requested.append(normalize_sha512(value))
    ...
    def put_cached_block(self,digest:str,data:bytes)->Path:
        ...
        if sha512_bytes(data)!=digest: raise ValueError("block sha512 mismatch")
        target=self.cache_path(digest); target.parent.mkdir(parents=True,exist_ok=True)
        if not target.exists():
            temporary=target.with_suffix(".tmp"); temporary.write_bytes(data); os.replace(temporary,target)
        self.record_event("block_cached",sha512=digest,detail={"length":len(data)}); return target

    def read_block(self,digest:str,expected_length:int|None=None)->bytes:
        digest=normalize_sha512(digest); cached=self.cache_path(digest)
        if cached.is_file() and not cached.is_symlink():
            data=cached.read_bytes()
            if sha512_bytes(data)==digest and (expected_length is None or len(data)==expected_length): return data
    ...
    def reconstruct(self,destination:Path,manifest:dict[str,Any])->dict[str,int]:
        if not content_manifest_valid(manifest): raise ValueError("invalid content block manifest")
        destination=Path(destination); destination.parent.mkdir(parents=True,exist_ok=True); reused=0
        with destination.open("wb") as target:
            for block in manifest["blocks"]:
                data=self.read_block(block["sha512"],int(block["length"])); target.write(data); reused+=len(data)
        whole=hashlib.sha512(destination.read_bytes()).hexdigest()
        if whole!=normalize_sha512(manifest["file_sha512"]): destination.unlink(missing_ok=True); raise ValueError("reconstructed file sha512 mismatch")
    ...
    def record_event(self,action:str,*,sha512:str="",relative_path:str="",detail:dict[str,Any]|None=None)->None:
        with self._db() as db: db.execute("INSERT INTO block_event(action,sha512,relative_path,detail_json,created_at) VALUES(?,?,?,?,?)",(action[:120],sha512[:128],relative_path[:1000],json.dumps(detail or {},ensure_ascii=False),int(time.time())))

    def stats(self)->dict[str,int]:
        with self._db() as db:
            sources=int(db.execute("SELECT COUNT(*) FROM block_source").fetchone()[0]); unique=int(db.execute("SELECT COUNT(DISTINCT sha512) FROM block_source").fetchone()[0]); manifests=int(db.execute("SELECT COUNT(*) FROM file_manifest").fetchone()[0])
        cached=sum(1 for path in self.cache.glob("*/*/*.block") if path.is_file()); return {"block_sources":sources,"unique_blocks":unique,"file_manifests":manifests,"cached_blocks":cached}
