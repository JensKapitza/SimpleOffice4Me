...
def _peer_allowed(peer_id: str, action: str) -> bool:
    ...
    return bool(peer and peer.get("enabled") and isinstance(section, dict) and section.get(action) is True)


@federation_bp.get("/manifest")
def network_boot_manifest():
    source_peer = request.headers.get("X-SimpleOffice-Peer-ID", "").strip()[:128]
    if source_peer and not _peer_allowed(source_peer, "receive"):
        return jsonify({"error": "network_boot_not_allowed"}), 403
...
@federation_bp.put("/assets/<path:relative>")
def receive_network_boot_asset(relative: str):
    source_peer = request.headers.get("X-SimpleOffice-Peer-ID", "").strip()[:128]
    if not _peer_allowed(source_peer, "receive"):
        return jsonify({"error": "network_boot_receive_not_allowed"}), 403
    ...
    FederationStore(current_app.config["DOCUMENT_ROOT"]).record_event("network_boot_asset_received", peer_id=source_peer, detail=result)
    return jsonify(result), 201


@federation_bp.put("/settings")
def receive_network_boot_settings():
    source_peer = request.headers.get("X-SimpleOffice-Peer-ID", "").strip()[:128]
    if not _peer_allowed(source_peer, "receive"):
        return jsonify({"error": "network_boot_receive_not_allowed"}), 403
    body = request.get_json(silent=True)
    if not isinstance(body, dict): return jsonify({"error": "invalid_settings"}), 400
    try:
        clean = save_boot_settings(body, _config_path())
    except ValueError as exc:
        return jsonify({"error": "invalid_settings", "detail": str(exc)}), 400
    FederationStore(current_app.config["DOCUMENT_ROOT"]).record_event("network_boot_settings_received", peer_id=source_peer, detail={"profiles": len(clean["profiles"])})
    return jsonify({"saved": True, "profiles": len(clean["profiles"])})
