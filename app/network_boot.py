"""PXE/iPXE network boot storage and RFC 1350 TFTP service.

TFTP is intentionally used for small first-stage boot loaders only. Kernel,
initrd and ISO payloads are also exposed through HTTP by network_boot_http so
large transfers are not forced through lock-step TFTP.
"""
...
TFTP_ERROR_ACCESS = 2
TFTP_ERROR_ILLEGAL = 4
TFTP_ERROR_OPTION = 8
SAFE_COMPONENT = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._/-]{0,500}$")
PROFILE_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]{0,79}$")
ARCH_BOOT_FILES = {
    0: "undionly.kpxe",   # Intel x86PC / legacy BIOS
    6: "ipxe32.efi",      # EFI IA32
    7: "ipxe.efi",        # EFI BC
    9: "ipxe.efi",        # EFI x86-64
    11: "ipxe-arm64.efi", # EFI ARM64
}
...
def validate_boot_settings(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    data = dict(DEFAULT_BOOT_SETTINGS)
...
def store_asset(source, filename: str, config_path: str | Path | None = None, *, max_bytes: int = 16 * 1024 * 1024 * 1024) -> dict[str, Any]:
    ...
    target = safe_asset_path(relative, config_path, must_exist=False)
    ...
    try:
        with temporary.open("wb") as handle:
            while True:
                ...
                total += len(block)
                if total > max_bytes:
                    raise ValueError("Boot-Datei überschreitet das Größenlimit")
...
def pxe_dhcp_values(options: dict[int, bytes], config_path: str | Path | None = None) -> tuple[str, str]:
    ...
    if is_ipxe(options):
        ...
        return "", f"{base}/network-boot/ipxe{suffix}"
    arch = architecture_from_options(options)
    if arch == 11:
        loader = settings["uefi_arm64_loader"]
    elif arch in {6, 7, 9}:
        loader = settings["uefi_x64_loader"]
    else:
        loader = settings["bios_loader"]
    return "", loader


def render_ipxe(profile_id: str, config_path: str | Path | None = None, *, request_base: str = "") -> str:
    settings = load_boot_settings(config_path)
    profiles = {item["id"]: item for item in settings["profiles"] if item["enabled"]}
    selected = profile_id or settings["default_profile"]
    if not selected or selected not in profiles:
        raise ValueError("Kein aktives Bootprofil ausgewählt")
    profile = profiles[selected]
    base = settings["http_base_url"] or request_base.rstrip("/")
    if not base:
        raise ValueError("HTTP-Basis für Networkboot fehlt")
    file_base = base + "/network-boot/files/"
    lines = ["#!ipxe", f"# SimpleOffice4Me profile: {profile['label']}", "dhcp ||"]
    if profile["mode"] == "kernel":
        kernel = file_base + profile["kernel"]
        args = profile["kernel_args"]
        lines.append(f"kernel {kernel}{(' ' + args) if args else ''}")
        if profile["initrd"]:
            lines.append(f"initrd {file_base}{profile['initrd']}")
        lines.append("boot")
...
def federation_manifest(config_path: str | Path | None = None) -> dict[str, Any]:
    settings = load_boot_settings(config_path)
    return {"schema": "simpleoffice-network-boot/v1", "settings": settings, "assets": list_assets(config_path), "generated_at": int(time.time())}


def _parse_rrq(packet: bytes) -> tuple[str, str, dict[str, str]]:
    if len(packet) < 4 or struct.unpack_from("!H", packet)[0] != TFTP_RRQ:
        raise ValueError("Keine TFTP-RRQ")
    parts = packet[2:].split(b"\0")
    if len(parts) < 3 or not parts[0] or not parts[1]:
        raise ValueError("Ungültige TFTP-RRQ")
    filename = parts[0].decode("utf-8", errors="strict")
    mode = parts[1].decode("ascii", errors="strict").casefold()
    options: dict[str, str] = {}
    values = parts[2:]
    if values and values[-1] == b"": values = values[:-1]
...
class TftpService:
    ...
    def __init__(self, settings: dict[str, Any], config_path: Path, event: Callable[[dict[str, Any]], None] | None = None):
        self.settings = validate_boot_settings(settings)
    ...
    def start(self) -> None:
        ...
        sock.bind((self.settings["tftp_bind"], self.settings["tftp_port"]))
        sock.settimeout(1)
        self.socket = sock
    ...
    def _serve_rrq(self, packet: bytes, client: tuple[str, int]) -> None:
        try:
            ...
            path = safe_asset_path(filename, self.config_path)
            size = path.stat().st_size
            if size > int(self.settings["tftp_max_file_bytes"]):
                raise PermissionError("file too large for TFTP; use HTTP")
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as transfer:
            ...
            with path.open("rb") as source:
                ...
                while True:
                    ...
                    payload = struct.pack("!HH", TFTP_DATA, block_number & 0xFFFF) + data
    ...
    def _exchange(self, transfer: socket.socket, client: tuple[str, int], payload: bytes, *, expected_ack: int) -> bool:
        for _ in range(int(self.settings["tftp_retries"])):
            try:
                transfer.sendto(payload, client)
                reply, source = transfer.recvfrom(2048)
