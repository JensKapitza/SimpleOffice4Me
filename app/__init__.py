...
def configured_upload_limit_bytes() -> int:
    requested = os.environ.get("SIMPLEOFFICE_MAX_UPLOAD_MIB", str(DEFAULT_UPLOAD_LIMIT_MIB)).strip()
    try:
...
def configured_webdav_upload_scan() -> bool:
    return os.environ.get("SIMPLEOFFICE_WEBDAV_CLAMAV", "0").strip().casefold() in {"1", "true", "yes", "on"}


def configured_webdav_quarantine_bytes() -> int:
    requested = os.environ.get("SIMPLEOFFICE_WEBDAV_QUARANTINE_MIB", str(DEFAULT_WEBDAV_QUARANTINE_MIB)).strip()
    try:
        limit_mib = int(requested)
    except ValueError:
...
app.config['GOOGLE_OAUTH_AUTO_PROVISION'] = os.environ.get('SIMPLEOFFICE_GOOGLE_AUTO_PROVISION', '0').strip().casefold() in {'1','true','yes','on'}
app.config['ALLOW_PUBLIC_REGISTRATION'] = os.environ.get('SIMPLEOFFICE_ALLOW_PUBLIC_REGISTRATION', '0').strip().casefold() in {'1','true','yes','on'}
...
def request_too_large(_error):
    ...
    return (f"Anfrage zu groß. Das konfigurierte Upload-Limit beträgt {limit_mib} MiB.\n", 413, {"Content-Type":"text/plain; charset=utf-8"})
...
def assign_request_id():
    g.request_id = secrets.token_hex(8); g.request_started_at = time.perf_counter()

@app.before_request
def verify_browser_request(): protect_browser_mutation()
...
def publish_request_id(response):
    ...
    if started is not None:
        ...
        log("request_performance request_id=%s endpoint=%s method=%s status=%s duration_ms=%.1f",getattr(g,"request_id",""),request.endpoint or "-",request.method,response.status_code,duration_ms)
    return response

@app.errorhandler(403)
def forbidden(error): return render_template("errors/403.html",message=getattr(error,"description","Zugriff verweigert.")),403

@app.errorhandler(Exception)
def unhandled_application_error(error):
    if isinstance(error,HTTPException): return error
    if app.testing and app.config.get("PROPAGATE_EXCEPTIONS",True): raise error
    request_id=getattr(g,"request_id",secrets.token_hex(8)); exception_type=type(error).__name__[:120]; endpoint=(request.endpoint or "")[:160]; path=request.path[:500]
    extracted_frames=traceback.extract_tb(error.__traceback__)[-12:]; template,template_line,undefined_variable=_jinja_error_context(error,extracted_frames)
    frames=[{"file":Path(frame.filename).name[:160],"line":frame.lineno,"function":frame.name[:160]} for frame in extracted_frames]
    if template: frames.append({"template":template,"line":template_line,"variable":undefined_variable})
    try:
        from .access_control import error_fingerprint,utc_now
        from .db import get_db
        actor=getattr(g,"user",None); dbh=get_db(); dbh.execute("""INSERT OR IGNORE INTO application_error(occurred_at, request_id, actor_id, exception_type, endpoint, method, path, fingerprint, frames) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",(utc_now(),request_id,actor["id"] if actor else None,exception_type,endpoint,request.method[:12],path,error_fingerprint(exception_type,endpoint,request.method[:12],path),json.dumps(frames,ensure_ascii=False))); dbh.commit()
    except Exception: pass
    app.logger.error("Unhandled application error request_id=%s type=%s endpoint=%s template=%s line=%s variable=%s",request_id,exception_type,endpoint,template or "-",template_line or "-",undefined_variable or "-")
    return render_template("errors/500.html",request_id=request_id),500

@app.before_request
def load_interface_preferences():
    settings=SettingsStore(app.config["DOCUMENT_ROOT"]).settings(); language=session.get("simpleoffice_language",settings["interface"]["default_language"]); g.language=language if language in ("de","en") else settings["interface"]["default_language"]
    user_theme=str(g.user["theme"] or "system") if getattr(g,"user",None) is not None else "system"; g.theme_preference=user_theme if user_theme in {"light","dark","system"} else "system"; g.app_settings=settings

@app.context_processor
def template_preferences():
    language=getattr(g,"language","de"); return {"tr":lambda key:translate(language,key),"ui_literal_translations":ui_literal_translations(language),"csrf_token":csrf_token}

@app.template_filter('datetime')
def format_datetime(value,format='%Y-%m-%d'): return value.strftime(format)

@app.template_filter("calendar_input_datetime")
def calendar_input_datetime(value):
    if not value: return ""
    try: return datetime.datetime.fromisoformat(str(value).replace("Z","+00:00")).strftime("%Y-%m-%dT%H:%M")
...
def calendar_display_datetime(value):
    if not value: return ""
    try:
        parsed=datetime.datetime.fromisoformat(str(value).replace("Z","+00:00")); suffix=parsed.strftime(" %Z") if parsed.tzname() else ""; return parsed.strftime("%d.%m.%Y, %H:%M")+suffix
...
def add_header(response):
    ...
    if request.endpoint=="service_worker": response.headers["Cache-Control"]="no-cache"; response.headers.pop("Expires",None)
    elif not app.debug and ("text/css" in str(response.content_type) or "application/javascript" in str(response.content_type)):
        then=datetime.datetime.now()+datetime.timedelta(minutes=5); response.headers["Cache-Control"]="public,max-age=1000"; response.headers["Expires"]=then.strftime("%a, %d %b %Y %H:%M:%S GMT")
    response.headers.setdefault("X-Content-Type-Options","nosniff"); response.headers.setdefault("X-Frame-Options","SAMEORIGIN"); response.headers.setdefault("Referrer-Policy","same-origin")
    camera_policy="camera=(self)" if request.blueprint in {"inventory","library"} else "camera=()"; response.headers.setdefault("Permissions-Policy",f"{camera_policy}, microphone=(), geolocation=()")
    response.headers.setdefault("Cross-Origin-Opener-Policy","same-origin"); response.headers.setdefault("Cross-Origin-Resource-Policy","same-origin")
...
def staticfile(dirname="",filename=""): return download_file(static_dir,dirname,filename)

@app.get('/service-worker.js')
def service_worker():
    response=send_from_directory(static_dir,"service-worker.js",mimetype="application/javascript"); response.headers["Cache-Control"]="no-cache"; response.headers["Service-Worker-Allowed"]="/"; return response

@app.route('/<myFile>',methods=['GET','POST'])
@app.route('/<lang>/<myFile>',methods=['GET','POST'])
def index(myFile="index.html",lang=None):
    if lang is not None: g.current_lang=lang
    try: return renderwithbs4(myFile)
    except TemplateNotFound: abort(404)

@app.get('/')
def home():
    if getattr(g,"user",None) is None: return redirect(url_for("auth.login"))
    return redirect(url_for("documents.dashboard"))

if __name__=='__main__': print("startup using flask internal or gunicorn3 -b :80 app ")
