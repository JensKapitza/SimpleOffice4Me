...
- Pro Benutzer ein separat erzeugtes WebDAV-App-Passwort. Das normale Login-Passwort und OAuth-Token werden nicht an LibreOffice gegeben.
- Die bestehende Upload-Grenze `SIMPLEOFFICE_MAX_UPLOAD_MIB` gilt auch für `PUT`.

Der Link enthält weder Passwort noch Session-Cookie. Ein neues App-Passwort ersetzt das alte sofort. **WebDAV deaktivieren** widerruft den Zugriff; bereits geöffnete lokale Arbeitskopien werden dadurch nicht gelöscht.
...
- `MKCOL`, `DELETE`, `MOVE` und `COPY`: LibreOffice darf Inhalte bearbeiten, aber keine Dokumente oder Ordner über WebDAV anlegen, verschieben oder löschen.
...
**WebDAV deaktivieren** widerruft das App-Passwort sofort. Das Entfernen des WebDAV-Blueprints stellt das frühere Verhalten ohne Änderung der Dokumente wieder her. Versionsdateien sollten erst nach einer geprüften Sicherung manuell entfernt werden; die Funktion ändert keine Aufbewahrungsregel und löscht keine Revision automatisch.
